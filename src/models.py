...
def model(dataset_name,df,name_target, n,contamination,number_of_unique,percentage_values,list_of_models):
    ...
    if 'LOF_pyod' in list_of_models:
      ...
      seed = 120
      test_size = 0.3
      X_train_lof, X_test_lof, y_train_lof, y_test_lof = train_test_split(X_lof, y_lof, test_size=test_size, random_state=seed,stratify=y_lof)
      ...
      # get the prediction on the test data
      y_test_pred = clf.predict(X_test_lof)  # outlier labels (0 or 1)
      y_test_scores_lof = clf.decision_function(X_test_lof)  # outlier scores
      ...
      #AUC score
      auc_4 = metrics.roc_auc_score(y_test_lof, predictions_4)
      #{:.3%}".format(auc_1)

      # AP score
      average_precision_4 = metrics.average_precision_score(y_test_lof, predictions_4)
      ...
      # evaluate the classification_report
      print(classification_report(y_test_lof, predictions_4))
      ...
      confusion_matrix_plot(y_test_lof,predictions_4,model_name_4,contamination,percentage_values)
    ...
    if 'HBOS' in list_of_models:
      ...
      confusion_matrix_plot(y_test,predictions_5h,model_name_5,contamination,percentage_values)
...
#***************************************************************SOM
    ...
    # # print(X.shape)
    # som = SOM(n_columns=20, n_rows=100, random_state=1)
    ...
    if 'LOF_sklearn' in list_of_models:
      ...
      neigh = LocalOutlierFactor(n_neighbors=3,novelty=True, contamination=contamination)
      start = time.time()
      neigh.fit(X_train_lof)

      # get the prediction on the test data
      y_test_pred_9 = neigh.predict(X_test_lof)
      ...
      #AUC score
      auc_9 = metrics.roc_auc_score(y_test_lof, predictions_9)
      # "{:.3%}".format(auc_6)

      # AP score
      average_precision_9 = metrics.average_precision_score(y_test_lof, predictions_9)
      ...
      # evaluate the classification_report
      print(classification_report(y_test_lof, predictions_9))
      ...
      confusion_matrix_plot(y_test_lof,predictions_9,model_name_9,contamination,percentage_values)
...
#***************************************************************************mcd
    ...
    if 'LOF_pyod' in list_of_models:
      fpr, tpr, thresh = metrics.roc_curve(y_test_lof, predictions_4)
    ...
    if 'LOF_sklearn' in list_of_models:
      fpr, tpr, thresh = metrics.roc_curve(y_test_lof, predictions_9)
      auc = metrics.roc_auc_score(y_test_lof, predictions_9)
      plt.plot(fpr,tpr,marker='.',label="LOF_sklearn, auc="+str("{:.3%}".format(auc)))
    ...
    if 'LOF_pyod' in list_of_models:
      disp_4 = my_plot_precision_recall_curve(y_test_lof, predictions_4,"LOF_pyod")
    ...
    if 'LOF_sklearn' in list_of_models:
      disp_9 = my_plot_precision_recall_curve(y_test_lof, predictions_9,"LOF_sklearn")
      average_precision_9 = metrics.average_precision_score(y_test_lof, predictions_9)
      disp_9.ax_.set_title('2-class Precision-Recall curve')
