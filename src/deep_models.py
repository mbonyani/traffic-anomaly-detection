...
def deep_model(dataset_name,df,name_target,n,EPOCH):
    ...
    PATH = f'/content/{MODEL_SELECTED}_{dataset_name}_{now.strftime("%Y-%m-%d %H-%M-%S")}.pt'
    ...
    #AUC score
    auc_1 = metrics.roc_auc_score(y_test_asli, predictions_1)
    ...
    # #************************************************
    df_all.loc[numb]= [f"{MODEL_SELECTED}",tn, fp, fn, tp, accuracy_1, precision_1,recall_1,
                       f1_score_1,"{:.3%}".format(auc_1),"{:.2f}".format(average_precision_1),train_time_deepant]
    numb = len(df_all)+1
    ...
    #AUC score
    auc_2 = metrics.roc_auc_score(y_test_asli, predictions_2)
    ...
    df_all.loc[numb]= [f"{MODEL_SELECTED}",tn, fp, fn, tp, accuracy_2, precision_2,recall_2,
                       f1_score_2,"{:.3%}".format(auc_2),"{:.2f}".format(average_precision_2),train_time_lstmae]
    numb = len(df_all)+1
    ...
    plt.plot(loss_test_deepant)
    plt.title('DeepAnT model loss_test')
    plt.ylabel('loss')
    plt.xlabel('epoch')
    #plt.ylim(.0, .50)
    plt.legend(['test'],bbox_to_anchor=(1.04,1), loc='upper left')
    plt.show()
    ...
    plt.plot(loss_test_lstmae)
    plt.title('LSTM-AE model loss_test')
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.legend(['test'],bbox_to_anchor=(1.04,1), loc='upper left')
    plt.show()
    ...
    fpr, tpr, thresh = metrics.roc_curve(y_test_asli, predictions_2)
    auc = metrics.roc_auc_score(y_test_asli, predictions_2)
    plt.plot(fpr,tpr,label="LSTM-AE, auc="+str("{:.3%}".format(auc)))
